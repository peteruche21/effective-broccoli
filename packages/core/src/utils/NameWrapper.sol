...
import {ERC1155Fuse, IERC165, IERC1155MetadataURI} from "@ens/contracts/wrapper/ERC1155Fuse.sol";
import {Controllable} from "@ens/contracts/wrapper/Controllable.sol";
import {INameWrapper, CANNOT_UNWRAP, CANNOT_BURN_FUSES, CANNOT_TRANSFER, CANNOT_SET_RESOLVER, CANNOT_SET_TTL, CANNOT_CREATE_SUBDOMAIN, CANNOT_APPROVE, PARENT_CANNOT_CONTROL, CAN_DO_EVERYTHING, CAN_EXTEND_EXPIRY, PARENT_CONTROLLED_FUSES, USER_SETTABLE_FUSES} from "@ens/contracts/wrapper/INameWrapper.sol";
    ...
    ReverseClaimer
{
    ...
    uint32 constant IS_DOT_BIT = 1 << 17;

    uint64 private constant GRACE_PERIOD = 90 days;
    bytes32 private constant BIT_NODE =
        0xfc602f00234da74893ed324e3aa56a3f7fe5cc12557d414fc4e44ab482fe1408;
    bytes32 private constant BIT_LABELHASH =
        0xb40c772e81d6bd6be343e15a1ede6fa38a54cbc83a1ee3eaac12661e1c4a1da6;
    ...
    constructor(
        ...
        /* Burn PARENT_CANNOT_CONTROL and CANNOT_UNWRAP fuses for ROOT_NODE and BIT_NODE and set expiry to max */
     ...
     * @param id Label as a string of the .bit domain to wrap
    ...
    function getApproved(
        uint256 id
    ) public view override(ERC1155Fuse, INameWrapper) returns (address operator) {
    ...
    function uri(
        uint256 tokenId
    ) public view override(INameWrapper, IERC1155MetadataURI) returns (string memory) {
    ...
    /**
     * @notice Wraps a .bit domain, creating a new token and sending the original ERC721 token to this contract
     * @dev Can be called by the owner of the name on the .bit registrar or an authorized caller on the registrar
     * @param label Label as a string of the .bit domain to wrap
    ...
    function wrapETH2LD(
        ...
        if (registrant != msg.sender && !registrar.isApprovedForAll(registrant, msg.sender)) {
            revert Unauthorised(_makeNode(BIT_NODE, bytes32(tokenId)), msg.sender);
        }
    ...
    /**
     * @dev Registers a new .bit second-level domain and wraps it.
     *      Only callable by authorized controllers.
     * @param label The label to register (Eg, 'foo' for 'foo.bit').
     * @param wrappedOwner The owner of the wrapped name.
     * @param duration The duration, in seconds, to register the name for.
     * @param resolver The resolver address to set on the ENS registry (optional).
     * @param ownerControlledFuses Initial owner-controlled fuses to set
     * @return registrarExpiry The expiry date of the new name on the .bit registrar, in seconds since the Unix epoch.
     */
    ...
    /**
     * @notice Renews a .bit second-level domain.
     * @dev Only callable by authorized controllers.
     * @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.bit').
     * @param duration The number of seconds to renew the name for.
     * @return expires The expiry date of the name on the .bit registrar, in seconds since the Unix epoch.
     */
    ...
    function renew(
        ...
        bytes32 node = _makeNode(BIT_NODE, bytes32(tokenId));
    ...
    /**
     * @notice Wraps a non .bit domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
     * @dev Can be called by the owner in the registry or an authorized caller in the registry
    ...
    /**
     * @notice Unwraps a .bit domain. e.g. vitalik.bit
     * @dev Can be called by the owner in the wrapper or an authorized caller in the wrapper
     * @param labelhash LabelHash of the .bit domain
     * @param registrant Sets the owner in the .bit registrar to this address
    ...
    function unwrapETH2LD(
        ...
        address registrant,
        address controller
    ) public onlyTokenOwner(_makeNode(BIT_NODE, labelhash)) {
        if (registrant == address(this)) {
            revert IncorrectTargetOwner(registrant);
        }
        _unwrap(_makeNode(BIT_NODE, labelhash), controller);
        registrar.safeTransferFrom(address(this), registrant, uint256(labelhash));
    }

    /**
     * @notice Unwraps a non .bit domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
     * @dev Can be called by the owner in the wrapper or an authoriszed caller in the wrapper
    ...
    /**
     * @notice Upgrades a domain of any kind. Could be a .bit name vitalik.bit, a DNSSEC name vitalik.xyz, or a subdomain
    ...
    function upgrade(bytes calldata _name, bytes calldata extraData) public {
        ...
        upgradeContract.wrapFromUpgrade(_name, currentOwner, fuses, expiry, approved, extraData);
    }
        ...
        operationAllowed(node, CANNOT_TRANSFER | CANNOT_SET_RESOLVER | CANNOT_SET_TTL)
    {
        ...
        if (owner == address(0)) {
            ...
            if (fuses & IS_DOT_BIT == IS_DOT_BIT) {
     ...
     * @param subnode Namehash of the sub name to check
     */

    function _checkCanCallSetSubnodeOwner(bytes32 parentNode, bytes32 subnode) internal view {
        (address subnodeOwner, uint32 subnodeFuses, uint64 subnodeExpiry) = getData(
            uint256(subnode)
        );
    ...
    function onERC721Received(
        ...
        //check if it's the bit registrar ERC721
        if (msg.sender != address(registrar)) {
            revert IncorrectTokenType();
        }

        (string memory label, address owner, uint16 ownerControlledFuses, address resolver) = abi
            .decode(data, (string, address, uint16, address));
    ...
    function _beforeTransfer(uint256 id, uint32 fuses, uint64 expiry) internal override {
        // For this check, treat .bit 2LDs as expiring at the start of the grace period.
        if (fuses & IS_DOT_BIT == IS_DOT_BIT) {
            expiry -= GRACE_PERIOD;
        }
    ...
    function _normaliseExpiry(
        ...
        // .bit names will check registrar, non .bit check parent
        if (expiry > maxExpiry) {
            expiry = maxExpiry;
        }
    ...
    function _wrapETH2LD(
        ...
        bytes32 labelhash = keccak256(bytes(label));
        bytes32 node = _makeNode(BIT_NODE, labelhash);
        // hardcode dns-encoded bit string for gas savings
        bytes memory _name = _addLabel(label, "\x03bit\x00");
        names[node] = _name;

        _wrap(node, _name, wrappedOwner, fuses | PARENT_CANNOT_CONTROL | IS_DOT_BIT, expiry);
    ...
    function _isETH2LDInGracePeriod(uint32 fuses, uint64 expiry) internal view returns (bool) {
        return fuses & IS_DOT_BIT == IS_DOT_BIT && expiry - GRACE_PERIOD < block.timestamp;
    }
}
